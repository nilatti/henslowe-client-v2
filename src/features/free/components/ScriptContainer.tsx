...
export function ScriptContainer({
  ...
  if (!text.lines || text.lines.length === 0) {
    return (
      <div className="text-sm text-gray-400 italic">No text selected.</div>
    );
  }
  ...
  const getPreviousCharacterId = (index: number) => {
    for (let j = index - 1; j >= 0; j--) {
      const prevId = sorted[j].character_id;
  ...
  const lineItems = sorted.map((line, i) => {
    const previousCharacterId = getPreviousCharacterId(i);
    const showCharacter = !!(
      line.character_id && line.character_id !== previousCharacterId
    );
