...
export function ConflictsManager({
  ...
  spaceId,
  canEdit,
}: ConflictsManagerProps) {
  const conflictKey = userId
    ? ["conflicts", { userId }]
    : ["conflicts", { spaceId }];
  ...
  const conflictOpts = (
    userId
      ? userConflictsQueryOptions(userId!)
      : spaceConflictsQueryOptions(spaceId!)
